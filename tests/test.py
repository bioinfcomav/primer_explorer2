...
import config
from combinations import divide_kmers_into_groups, get_compatible_groups_of_primers
from primer_xp import (parse_fasta, KmerLocationGenerator, GenomeRegion, get_pcr_products,
                       get_first_that_complies, GenomeRegions, select_primers_combinations,
                       HETEROCHROMATIN_BED, generate_kmer_locations, KmerAndLocation, filter_kmers_by_heterochromatin_stats,
                       get_top_kmers_by_minimum_abundance, filter_kmers_by_revcomp, KmerLocationGenerator,
                       PrimerAndLocation, annotate_reverse_complement_dimers, annotate_length_viable_products,
                       annotate_products_by_euchromatin_region, count_total_products)
from primer3 import kmer_validated_by_primer3, combinations_validated_by_primer3


class TestKmerGenerator(unittest.TestCase):

  def test_generate_kmers(self):
    heterochromatic_regions = GenomeRegions(open('test_hetero_prueba.bed', 'rb'))
    ...
    print(len(kmer_generator.kmer_counters[True].most_common(60)))
...
class TestSortByEuchromatinStats(unittest.TestCase):
  ...
  def test_sort_by_euchromatin_abundance(self):
    heterochromatic_regions = GenomeRegions(open('test_hetero_abundance.bed', 'rb'))

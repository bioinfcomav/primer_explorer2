...
def generate_pair_stats(primer_pair, count, short_products_cutoff=SHORT_PRODUCTS_CUTOFF):
    ...
    report.append("Number of nondimer and viable products (effective products):\t{}".format(str(count["non_dimer_viable_products"])))
    number_of_no_viable_products_removed = count["total_products"] - count["viable_products"]
    report.append("Number of short products removed:\t{}\t{:.2f}".format(str(number_of_no_viable_products_removed),
                                                                         number_of_no_viable_products_removed / count["total_products"]))
    number_of_no_effective_products_removed = count["total_products"] - count["non_dimer_viable_products"]
    report.append("Number of no dimer and short products removed (no effective products):\t{}\t{:.2f}".format(str(number_of_no_effective_products_removed),
                                                                                                              number_of_no_effective_products_removed / count["total_products"]))
    ...
    if number_of_no_effective_products_removed / count["total_products"] >= short_products_cutoff:
